...
public class GlobalExceptionHandler {
    ...
    private Result createResult(BaseException e) {
        return Result.failure(e.getMessage(), e.getCode() == null ? 1 : e.getCode());
    }
    // 处理认证异常
    @ExceptionHandler(AuthenticationException.class)
    public Result<Void> handleAuthenticationException(AuthenticationException e){
        e.printStackTrace();
        return Result.failure(StringUtils.hasLength(e.getMessage()) ? e.getMessage() : "认证失败！", 401);
    }
    ...
    public Result<Void> handleAccessDeniedException(AccessDeniedException e){
        e.printStackTrace();
        return Result.failure(StringUtils.hasLength(e.getMessage()) ? e.getMessage() : "权限不足！");
    }
    ...
    public Result handleVaildException(MethodArgumentNotValidException e){
        ...
        return Result.failure(stringBuffer + "");
    }
